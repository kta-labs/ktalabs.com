(function () {
  ...
  const CONTACT_HTML = `
    <div class="kta-chat-contact">
      <span>Toujours pas de réponse ? Contacte-nous directement :</span>
  ...
  style.textContent = `
    ...
    .kta-chat-panel {
      position: fixed; bottom: 152px; right: 20px; z-index: 500;
    ...
    @media (max-width: 640px) {
      .kta-chat-btn { bottom: 20px; right: 16px; }
      .kta-chat-panel {
        top: 70px; bottom: 90px; right: 12px; left: 12px;
  ...
  panel.querySelector('.kta-chat-close').addEventListener('click', closePanel);
